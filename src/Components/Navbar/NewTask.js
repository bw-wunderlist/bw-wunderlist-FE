  ...
  createTask = e => {
    ...
    axios
      .post(`https://wunderlist2.herokuapp.com/api/tasks`, {
        ...this.state.task,
        repeat_condition: JSON.stringify(this.state.repeat_condition)
      })
      .then(res => {
        this.setState({
          ...this.state,
        ...
        <ModalBody>
          <form onSubmit={e => this.addHandler(e)}>
            ...
            {this.state.task.repeat ? (
              <Row>
                <Col xs="12">
                  <p>Repeat Every {this.state.repeat_condition.number} {this.state.printRepeat} for {this.state.repeat_condition.number} times</p>
                </Col>
                <Col>
                  <FormGroup>
                    <Input
